...
subroutine ICPertFLRW_GRH_ICCalc (CCTK_ARGUMENTS)
  ...
  if (CCTK_EQUALS (ICPertFLRW_GRH_type_of_matter, "radiation")) then
    omega = 1._dp / 3._dp
    cmb_temp_today = 2.7_dp ! Kelvin
  else
    omega = 0._dp
  endif
  ...
  if (want_pflrw_spacetime .and. want_pflrw_hydro) then
    ...
    t0_EdS = 2._dp / ( 3._dp * H0 * (1._dp + omega))

    t = cctk_time
    aa = a0 * ( t / t0_EdS )**(2._dp/(3._dp * (1._dp + omega)))
    Hprop = H0 * t0_EdS / t
    Omega_matter = 1.0
    Lambda = 0._dp
    ...
    F = f_Loc + (3._dp/2._dp) * Omega_matter * ( 1._dp + omega )
    iFH = 1._dp / ( F * Hprop )
    iFH2 = 1._dp / ( F * Hprop**2._dp )
    m2iFH2 = - 2._dp * iFH2

    kappa = 8._dp * pi * ICPertFLRW_GRH_G 
    ...
    do k = 1, cctk_lsh(3)
      do j = 1, cctk_lsh(2)
        do i = 1, cctk_lsh(1)
          ...
          energy_density = (RicciScalar + K_Loc**2._dp - KijKji - 2._dp*Lambda) / (2._dp * kappa)
          ! temp = 1._dp !cmb_temp_today * a0 / aa
          heat_capacity = 1._dp !specific heat capacity = Boltzmann constant?
          eps(i,j,k) = ICPertFLRW_GRH_temp * heat_capacity
          rho(i,j,k) = energy_density / (1._dp + eps(i,j,k))
          press(i,j,k) = omega * rho(i,j,k) * eps(i,j,k)
          vel(i,j,k,:) = 0._dp
