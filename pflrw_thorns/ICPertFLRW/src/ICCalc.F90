...
subroutine ICPertFLRW_ICCalc (CCTK_ARGUMENTS)
  ...
  CCTK_REAL :: H0, a0, tau0_EdS, tauR_EdS, Omega_lambda0
  CCTK_REAL :: tau, aa, aa0, redshift, Hprop, Omega_matter, f_Loc
  CCTK_REAL :: eoswfac, kappa, mu
  CCTK_REAL :: kBdim, kBc, mass, TCMB, eps0, epsflrw
  ...
  logical :: want_pflrw, want_Lambda, want_radiation, want_conformal_time
  ...
  want_conformal_time = CCTK_EQUALS (ICPertFLRW_time, "conformal")
  ...
  if (want_pflrw) then
   ...
   ! Define the pressure to energy density ratio
    if (want_radiation) then
      call CCTK_INFO("Fluid is radiation")
      eosw = 1._dp / 3._dp
    else
      call CCTK_INFO("Fluid is dust")
      eosw = 0._dp
    endif
    ...
    ! eta0_EdS = 2._dp / (( eoswfac - 2._dp) * a0 * H0) ! Conformal time today
    Omega_lambda0 = 1._dp - ICPertFLRW_Omega_matter0 ! Dark energy parameter today
    kappa = 8._dp * pi * ICPertFLRW_G / ICPertFLRW_c**4._dp

    ! Define scale factor in terms of the reference redshift
    if (ICPertFLRW_z_comoving_ref .LT. 0._dp) then ! if reference redshift is negatif
    ...
    if (want_conformal_time) then
      if (want_Lambda) then
        call CCTK_INFO("TO DO: LambdaCDM with conformal time")
        tau = 0._dp
      else
    ...
    ! Define cosmological model of background
    if (want_Lambda) then
      ! Lambda CDM model
      call CCTK_INFO("Cosmological constant included")
      ...
      call CCTK_INFO("No cosmological constant")
    ...
    ! dimensionless Boltzmann constant
    ! kBdim = kB * G * H0 / (c**5) with
    ! H0 = 1e5 * Mpc and Mpc = 3.0857e22 meters
    kBdim = 1.17_dp * 10._dp ** (-48._dp) ! in Kelvin^-1
    ! Boltzmann constant in code units
    kBc = kBdim * (ICPertFLRW_c**4._dp) * 2997.9_dp / ICPertFLRW_G
    mass = 1 ! particle mass
    ! probably not right but internal energy of
    ! dust doesn't make sense so lets keep eps small
    TCMB = 2.725_dp ! CMB temperature in Kelvin
    eps0 = (3._dp/2._dp) * kBc * TCMB / mass ! Today's specific internal energy
    epsflrw = eps0 * (1._dp + redshift) ! specific internal energy
    ...
    do k = 1, cctk_lsh(3)
      do j = 1, cctk_lsh(2)
        do i = 1, cctk_lsh(1)
          ...
          ! energy density
          mu = (RicciScalar + K_Loc**2._dp - KijKji - 2._dp*Lambda) / (2._dp * kappa)
          ! specific internal energy
          eps(i,j,k) = epsflrw
          ! rest mass energy density
          rho(i,j,k) = mu / (1 + eps(i,j,k))
          ! pressure
          press(i,j,k) = eosw * rho(i,j,k)
