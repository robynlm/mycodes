...
subroutine ICPertFLRW_ICCalc (CCTK_ARGUMENTS)
  ...
  logical :: want_pflrw, want_Lambda, want_radiation
  ...
  if (want_pflrw) then
    ...
    t0_EdS = 2._dp / ( 3._dp * H0 * (1._dp + eosw) )
    ...
    if (want_Lambda) then
      ...
      aa = a0 * ( t / t0_EdS )**(2._dp/(3._dp * (1._dp + eosw)))
      Hprop = H0 * t0_EdS / t
      Omega_matter = 1.0
      Lambda = 0._dp
    endif
    ...
    F = f_Loc + (3._dp/2._dp) * Omega_matter * ( 1._dp + eosw )
    iFH = 1._dp / ( F * Hprop )
