...
static void CosmoLapse_TraceK_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL KtraceL CCTK_ATTRIBUTE_UNUSED = kxxL*gu11 + kyyL*gu22 + 
      2*(kxyL*gu12 + kxzL*gu13 + kyzL*gu23) + kzzL*gu33;
    /* Copy local copies back to grid functions */
    Ktrace[index] = KtraceL;
  }
  CCTK_ENDLOOP3(CosmoLapse_TraceK);
}
extern "C" void CosmoLapse_TraceK(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_TraceK
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_TraceK);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_TraceK_Body");
  }
  if (cctk_iteration % CosmoLapse_TraceK_calc_every != CosmoLapse_TraceK_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "CosmoLapse_TraceK", 3, groups);
  ...
  LoopOverEverything(cctkGH, CosmoLapse_TraceK_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_TraceK_Body");
  }
}
