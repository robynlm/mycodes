...
static void CosmoLapse_dtLapsePostStep_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL KtraceaverageL CCTK_ATTRIBUTE_UNUSED = Ktraceaverage[index];
    CCTK_REAL KtraceaverageiniL CCTK_ATTRIBUTE_UNUSED = 
      Ktraceaverageini[index];
    ...
    CCTK_REAL Kexp CCTK_ATTRIBUTE_UNUSED = IfThen(Kexpression == 1, -2.*pow(tauiniL,-1),
                                                  IfThen(Kexpression == 2, -2.*pow(tauL,-1),
                                                         IfThen(Kexpression == 3, KiniL*tauiniL*pow(tauL,-1),
                                                                IfThen(Kexpression == 4, KtraceaverageL,
                                                                       IfThen(Kexpression == 5, KiniL*KtraceaverageL*pow(KtraceaverageiniL,-1),0.)))));
    
    CCTK_REAL Kn CCTK_ATTRIBUTE_UNUSED = IfThen(Knorm == 
      1,2.*pow(tauL,-1),IfThen(Knorm == 2,tauiniL*pow(tauL,-1),IfThen(Knorm 
      == 3,KtraceaverageL*pow(KtraceaverageiniL,-1),1.)));
    
    CCTK_REAL partialtalpha CCTK_ATTRIBUTE_UNUSED = 
      IfThen(alphaFullLieDeriv != 0,betaxL*JacPDupwindNthAnti1alp + 
      ...
      JacPDupwindNthSymm3alp*fabs(betazL),0) + alphaF*(-KtraceL + 
      Kexp)*pow(alpL,alphaN)*pow(Kn,-1);
