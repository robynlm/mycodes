...
static void ProperTime_RHS_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_ENDLOOP3(ProperTime_RHS);
}
extern "C" void ProperTime_RHS(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ProperTime_RHS_Body");
  }
  if (cctk_iteration % ProperTime_RHS_calc_every != ProperTime_RHS_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "ProperTime_RHS", 4, groups);
  
  
  LoopOverEverything(cctkGH, ProperTime_RHS_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ProperTime_RHS_Body");
  }
}
