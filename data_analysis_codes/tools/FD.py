...
class FD_Class():
    ...
    def Dt(self, f, dt):
        return np.array([self.FD.backward(f, it, dt) 
                         for it in range(self.mask*2,len(f))])
    
    def D3x(self, f): # 3D Derivative along x
        if self.pBound:
            flong = np.concatenate((f[-self.mask:, :, :], f, 
                                    f[:self.mask, :, :]), axis=0)
            return np.array([self.FD.centered(flong, ix, self.dx) 
                             for ix in range(self.mask, len(f)+self.mask)])
        else:
            N = len(f)
            LHS = np.array([self.FD.forward(f, ix, self.dx) 
                            for ix in range(0, self.mask)])
            central_part = np.array([self.FD.centered(f, ix, self.dx) 
                                     for ix in range(self.mask, N-self.mask)])
            RHS = np.array([self.FD.backward(f, ix, self.dx) 
    ...
    def D3y(self, f): # 3D Derivative along y
        ...
        if self.pBound:
            flong = np.concatenate((f[:, -self.mask:, :], f, 
                                    f[:, :self.mask, :]), axis=1)
            return np.array([[self.FD.centered(flong[ix,:,:], iy, self.dy) 
                              for iy in range(self.mask, N+self.mask)] 
                             for ix in range(N)])
        else:
            LHS = np.array([[self.FD.forward(f[ix,:,:], iy, self.dy) 
                             for iy in range(0, self.mask)] for ix in range(N)])
            central_part = np.array([[self.FD.centered(f[ix,:,:], iy, self.dy) 
                                      for iy in range(self.mask, N-self.mask)] 
                                     for ix in range(N)])
            RHS = np.array([[self.FD.backward(f[ix,:,:], iy, self.dy) 
    ...
    def D3z(self, f): # 3D Derivative along z
        ...
        if self.pBound:
            flong = np.concatenate((f[:, :, -self.mask:], f, 
                                    f[:, :, :self.mask]), axis=2)
            return np.array([[[self.FD.centered(flong[ix,iy,:], iz, self.dz) 
                               for iz in range(self.mask, N+self.mask)] 
                              for iy in range(N)] 
                             for ix in range(N)])
        else:
            LHS = np.array([[[self.FD.forward(f[ix,iy,:], iz, self.dz) 
                              for iz in range(0, self.mask)] 
                             for iy in range(N)] 
                            for ix in range(N)])
            central_part = np.array([[[self.FD.centered(f[ix,iy,:], iz, self.dz) 
                                       for iz in range(self.mask, N-self.mask)] 
                                      for iy in range(N)] 
                                     for ix in range(N)])
            RHS = np.array([[[self.FD.backward(f[ix,iy,:], iz, self.dz) 
