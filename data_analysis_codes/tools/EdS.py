...
class evo:
    def __init__(self, matter='dust'):
        ...
        self.t_today = 2.0 / (3.0 * self.Hprop_today * (1.0 + self.omega))
        self.Lambda = 0.0
        self.Omega_l_today = 0.0

    def a(self, t):
        """Scale factor"""
        return self.a_today * ((t / self.t_today) ** (2 / (3 * (1.0 + self.omega))))
    
    def t_func_a(self, a):
        """Proper time from scale factor"""
        return self.t_today * ((a / self.a_today) ** ((3 * (1.0 + self.omega)) / 2))
