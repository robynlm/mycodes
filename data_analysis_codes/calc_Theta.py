...
class MyClass:
    ...
    def getTheta(self, it):
        ...
        all_rls = set([int(k.split('rl=')[1]) 
                       ...
                       if varname in k])
        for rl in all_rls:
            working_keys = [k for k in list(f.keys()) 
                            if 'rl='+str(rl) in k]
            ...
            if (theta_key not in working_keys) or self.recalculate:
                ...
                #  Curvature
                #---------------------------------------------------
                Gudd3, Gddd3 = self.RCW.Christoffel_symbol(
                    gdown, gup)
                ...
                Kup = np.einsum(
                    'im...,jn...,ij...->mn...', gup, gup, Kdown)
                dbetaup = self.FD.D3_tensor1(betaup)
                Lbgup = (np.einsum(
                    's...,sij...->ij...', 
                    betaup, self.FD.D3_tensor2(gup))
                         - np.einsum(
                             'si...,sj...->ij...', dbetaup, gup)
                         - np.einsum(
                             'sj...,is...->ij...', dbetaup, gup))
                ...
                dtD = - np.einsum(
                    'ii...->...', self.FD.D3_tensor1(D * V))
                dtS = (- np.einsum(
                    'jij...->...', self.FD.D3_tensor2(
                        np.einsum('i...,j...->ij...', 
                                  S4down[1:], V))) 
                       + (S4down[0]*self.FD.D3_scalar(g4down400)/2)
                       + np.einsum(
                           'j...,ij...->i...', S4up[1:], 
                           self.FD.D3_tensor1(betadown))
                       + (np.einsum('j...,k...,ijk...->i...', 
                                    S4up[1:], S4up[1:], 
                                    self.FD.D3_tensor2(gdown))
                          / (2 * S4down[0]))
                ...
                par2 = (dtD
                        - np.einsum(
                            'ii...->...', self.FD.D3_tensor1(E * V))
                        - press * np.einsum(
                            'ii...->...', self.FD.D3_tensor1(
                                W * sgdet * V)))
                ...
                dtE = (- np.einsum(
                    'ii...->...', self.FD.D3_tensor1(E * V))
                       - press*(dtsgdetW + np.einsum(
                           'ii...->...', 
                           self.FD.D3_tensor1(sgdet*W*V))))
                del dtW, V, dtsgdet, dtsgdetW, sgdet, press

                # define time derivative of fluid velocity, 
                # \partial_t(u_\mu)
                dtudown3 = udown4[1:] * (
                    RRead.safe_division(dtS, S4down[1:])
                    - ((dtD + dtE) / (D + E)))
                dtudown0 = (((np.einsum(
                    'ij...,i...,j...->...',
                ...
                # delete variables unused in 
                # the rest of the code for memory
                del W, uup4, dtgup
                ...
                #---------------------------------------------------
                Gudd4 = self.RCW.Christoffel_symbol4beta(
                    alpha, dtalpha, betaup, dtbetaup, 
                    Kdown, gup, gdown, Gddd3)
                del alpha, dtalpha, betaup, dtbetaup
                del Kdown, gup, gdown, Gddd3

                #---------------------------------------------------
                # Fluid projection tensor
                #---------------------------------------------------
                hdown4 = g4down4 + np.einsum(
                    'a...,b...->ab...', udown4, udown4)
                hmixed4 = np.einsum(
                    'ac...,cb...->ab...', g4up4, hdown4)
                hup4 = np.einsum(
                    'ac...,bd...,cd...->ab...', 
                    g4up4, g4up4, hdown4)
                del g4down4
                ...
                omegadown = self.RCW.antisymmetric_tensor(
                    CovariantCovDu)
                omegaup = np.einsum(
                    'im...,jn...,mn...->ij...',
                    g4up4, g4up4, omegadown)
                omegamag = np.einsum(
                    'ij...,ij...->...', omegaup, omegadown) / 2
                ...
                added_vars = ['Theta', 
                              ...
                              'omegamag']
                allvar = [Theta, 
                          sheardown[0,0], sheardown[0,1], 
                          sheardown[0,2],  sheardown[1,1], 
        ...
        print(self.param['simname'] 
              ...
              + ' Progress = {:.2f}%'.format(percent_done), 
              flush=True)
...
if __name__ == "__main__":
    ...
    print('number of iterations : ', int(nbr_iterations), 
          flush=True)
