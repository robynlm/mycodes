...
class MyClass:
    ...
    def getvals(self, it):
        ...
        # extract values from base chunk
        rl0_avail = np.sum([' rl=0' in k for k in list(f.keys())])
        if rl0_avail!=0:
            ...
            if ((self.get_key('ADMBASE', 'alp', it) in f.keys()) 
                ...
                if ((self.get_key('ADMBASE', 'betax', it) in f.keys()) 
                    and (self.get_key('TMUNUBASE', 'eTtt', it) in f.keys())):
                    betaup = np.array(
                        [self.get_1ddata(f, 'ADMBASE', 'betax', it),
                ...
                # rho in fluid frame equivalent to the prior
            ...
            Ham_energy_scale = np.sqrt(abs((RicciS)**2 
                                           + ((2/3)*K**2)**2 + (2*A2)**2 
