...
class MyClass():
    def __init__(self, Lin, OGpath, all_it):
        self.FD = FD_file.FD_Class(Lin.p.dx, periodic_boundary=True, order6=False)
        self.Lin = Lin
        self.box1 = np.ones((Lin.N, Lin.N, Lin.N))
        self.box0 = np.zeros((Lin.N, Lin.N, Lin.N))
        self.Abox0 = np.array([self.box0]*(len(all_it)-self.FD.mask*2))
        self.OGpath = OGpath
        self.it_file_name = OGpath+'all_iterations/'+Lin.p.sim_name
        self.all_it = all_it
        self.dt = Lin.p.dx*Lin.p.dtfac*Lin.p.h5_every
    ...
    def read(self, var, it):
        if it=='all':
            ...
            if var=='Kdown':
                ...
                kij_keys = ['ADMBASE::{} it={} tl=0 rl={}'.format(kij, it, self.Lin.p.refinement_level) for kij in 
                        ['kxx', 'kxy', 'kxz', 'kyy', 'kyz', 'kzz']]
                curv, Kdown_full = RRead.read_xyz(f, kij_keys)
                Kdown = RRead.cut2(Kdown_full, self.Lin.p.nbr_ghost, self.Lin.N)
                return Kdown
            if var=='gdown' or var=='gup':
                f = h5py.File('{}_it_{:06d}.hdf5'.format(self.it_file_name, it), 'r')
                gij_keys = ['ADMBASE::{} it={} tl=0 rl={}'.format(gij, it, self.Lin.p.refinement_level) for gij in 
                        ['gxx', 'gxy', 'gxz', 'gyy', 'gyz', 'gzz']]
                metric, gdown_full = RRead.read_xyz(f, gij_keys)
                gdown = RRead.cut2(gdown_full, self.Lin.p.nbr_ghost, self.Lin.N)
                if var=='gdown':
                    return gdown
                else:
                     return RRead.cut2(RRead.inv(metric, RRead.det(metric)), self.Lin.p.nbr_ghost, self.Lin.N), gdown
            if var in ['dtgdown4', 'dxgdown4', 'dygdown4', 'dzgdown4', 
    ...
    def record(self, varname, var, it, separate_iterations=True):
        if separate_iterations:
            if it=='all':
                for i, it in enumerate(self.all_it[self.FD.mask:-self.FD.mask]):
    ...
    def calc_dtdtgdown4(self):
        dtdtgxy = self.FD.D4t(self.read('dtgxy', 'all'), self.dt)
        dtdtgxz = self.FD.D4t(self.read('dtgxz', 'all'), self.dt)
        dtdtgyz = self.FD.D4t(self.read('dtgyz', 'all'), self.dt)
        self.record('dtdtgdown4', np.array([[self.Abox0, self.Abox0, self.Abox0, self.Abox0],
                               [self.Abox0, self.FD.D4t(self.read('dtgxx', 'all'), self.dt), dtdtgxy, dtdtgxz],
                               [self.Abox0, dtdtgxy, self.FD.D4t(self.read('dtgyy', 'all'), self.dt), dtdtgyz],
                               [self.Abox0, dtdtgxz, dtdtgyz, self.FD.D4t(self.read('dtgzz', 'all'), self.dt)]]), 'all')
    def calc_dtdxgdown4(self):
        dtdxgxy = self.FD.D4t(self.read('dxgxy', 'all'), self.dt)
        dtdxgxz = self.FD.D4t(self.read('dxgxz', 'all'), self.dt)
        dtdxgyz = self.FD.D4t(self.read('dxgyz', 'all'), self.dt)
        self.record('dtdxgdown4', np.array([[self.Abox0, self.Abox0, self.Abox0, self.Abox0],
                               [self.Abox0, self.FD.D4t(self.read('dxgxx', 'all'), self.dt), dtdxgxy, dtdxgxz],
                               [self.Abox0, dtdxgxy, self.FD.D4t(self.read('dxgyy', 'all'), self.dt), dtdxgyz],
                               [self.Abox0, dtdxgxz, dtdxgyz, self.FD.D4t(self.read('dxgzz', 'all'), self.dt)]]), 'all')
    def calc_dtdygdown4(self):
        dtdygxy = self.FD.D4t(self.read('dygxy', 'all'), self.dt)
        dtdygxz = self.FD.D4t(self.read('dygxz', 'all'), self.dt)
        dtdygyz = self.FD.D4t(self.read('dygyz', 'all'), self.dt)
        self.record('dtdygdown4', np.array([[self.Abox0, self.Abox0, self.Abox0, self.Abox0],
                               [self.Abox0, self.FD.D4t(self.read('dygxx', 'all'), self.dt), dtdygxy, dtdygxz],
                               [self.Abox0, dtdygxy, self.FD.D4t(self.read('dygyy', 'all'), self.dt), dtdygyz],
                               [self.Abox0, dtdygxz, dtdygyz, self.FD.D4t(self.read('dygzz', 'all'), self.dt)]]), 'all')
    def calc_dtdzgdown4(self):
        dtdzgxy = self.FD.D4t(self.read('dzgxy', 'all'), self.dt)
        dtdzgxz = self.FD.D4t(self.read('dzgxz', 'all'), self.dt)
        dtdzgyz = self.FD.D4t(self.read('dzgyz', 'all'), self.dt)
        self.record('dtdzgdown4', np.array([[self.Abox0, self.Abox0, self.Abox0, self.Abox0],
                               [self.Abox0, self.FD.D4t(self.read('dzgxx', 'all'), self.dt), dtdzgxy, dtdzgxz],
                               [self.Abox0, dtdzgxy, self.FD.D4t(self.read('dzgyy', 'all'), self.dt), dtdzgyz],
                               [self.Abox0, dtdzgxz, dtdzgyz, self.FD.D4t(self.read('dzgzz', 'all'), self.dt)]]), 'all')
    ...
    def calc_Weyl(self, it):
        ...
        self.record('RiemannTdown4', RiemannTdown4, it)
        RicciTdown4 = np.einsum('cacb...->ab...', RiemannTuddd4)
        del RiemannTuddd4
    ...
    def LeviCivita(self, gdown4): # 4 indices down
        LC = np.zeros((4, 4, 4, 4, self.Lin.N, self.Lin.N, self.Lin.N))
        for i0 in [0,1,2,3]:
...
if __name__ == "__main__":
    ...
    all_hdf5it = [int(i) for i in np.array(all_it[0::sim.h5_every])]
    funcs = MyClass(Lin, OGpath, all_hdf5it)
    it_counter = Value('i',0)
    pool = Pool(processes=int(sys.argv[2]), initializer = init, initargs = (it_counter, ))
    ...
    pool.map(funcs.calc_Weyl,  all_hdf5it[funcs.FD.mask:-funcs.FD.mask])
    print('Done!')   
