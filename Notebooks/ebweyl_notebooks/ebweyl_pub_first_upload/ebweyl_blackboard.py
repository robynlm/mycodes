...
class FiniteDifference:
    ...
    def d3x(self, f):
        ...
        if self.periodic_boundary:
            ...
            flong = np.concatenate(
                (f[-self.mask_len :, :, :], f, f[: self.mask_len, :, :]), axis=0
            )
            ...
            lhs = np.array(
                [self.forward(f, ix, self.inverse_dx) for ix in range(0, self.mask_len)]
            )
    ...
    def d3y(self, f):
        ...
        if self.periodic_boundary:
            flong = np.concatenate(
                (f[:, -self.mask_len :, :], f, f[:, : self.mask_len, :]), axis=1
            )
            return np.array(
                [
                    [
                        self.centered(flong[ix, :, :], iy, self.inverse_dx)
            ...
            lhs = np.array(
                [
                    [
                        self.forward(f[ix, :, :], iy, self.inverse_dx)
            ...
            central_part = np.array(
                [
                    [
                        self.centered(f[ix, :, :], iy, self.inverse_dx)
                        for iy in range(self.mask_len, self.N - self.mask_len)
                    ]
                    for ix in range(self.N)
                ]
            ...
            rhs = np.array(
                [
                    [
                        self.backward(f[ix, :, :], iy, self.inverse_dx)
    ...
    def d3z(self, f):
        ...
        if self.periodic_boundary:
            flong = np.concatenate(
                (f[:, :, -self.mask_len :], f, f[:, :, : self.mask_len]), axis=2
            )
            return np.array(
                [
                    [
                        [
                            self.centered(flong[ix, iy, :], iz, self.inverse_dx)
                            for iz in range(self.mask_len, self.N + self.mask_len)
                        ]
            ...
            lhs = np.array(
                [
                    [
                        [
                            self.forward(f[ix, iy, :], iz, self.inverse_dx)
            ...
            central_part = np.array(
                [
                    [
                        [
                            self.centered(f[ix, iy, :], iz, self.inverse_dx)
                            for iz in range(self.mask_len, self.N - self.mask_len)
                        ]
                        for iy in range(self.N)
                    ]
            ...
            rhs = np.array(
                [
                    [
                        [
                            self.backward(f[ix, iy, :], iz, self.inverse_dx)
    ...
    def d3_rank2tensor(self, f):
        ...
        return np.array(
            [
                [[self.d3x(f[k, j]) for j in range(3)] for k in range(3)],
                [[self.d3y(f[k, j]) for j in range(3)] for k in range(3)],
                [[self.d3z(f[k, j]) for j in range(3)] for k in range(3)],
            ]
        )
...
class Weyl:
    ...
    def eweyl_n_tensor_down3(self, Ricci3down3, kappa, Tdown4):
        ...
        gammamixed4 = gmixed4 + np.einsum("a..., c... -> ac...", self.ndown4, self.nup4)
        Sdown3 = np.einsum(
            "ca..., db..., cd... -> ab...", gammamixed4, gammamixed4, Tdown4
        )[1:, 1:]
    ...
    def bweyl_u_tensor_down4(self, Cdown4, uup4):
        ...
        LCuudd4 = np.einsum(
            ...
            self.gup4,
            self.gup4,
            self.levicivita_tensor_down4(),
        )
        Budown4 = (
            np.einsum(
                "b..., f..., abcd..., cdef... -> ae...", uup4, uup4, Cdown4, LCuudd4
            )
            / 2
        )
        return Budown4
    ...
    def bweyl_n_tensor_down3(self, Gudd3):
        ...
        Ktrace = self.trace_rank2tensor3(self.Kdown3)
        Kmixed3 = np.einsum("ij..., jk... -> ik...", self.gammaup3, self.Kdown3)
        Bterm2K = self.covariant_derivatice_3_scalar(Ktrace) - np.einsum(
            "ccb... -> b...", self.covariant_derivatice_3_tensor2mixed3(Gudd3, Kmixed3)
        )
        Bterm2 = (
            np.einsum("cdb..., ac..., d... -> ab...", LCuud3, self.gammadown3, Bterm2K)
            / 2
        )
    ...
    def ebweyl_n_3D_to_4D(self, fdown3):
        ...
        """
        fup3 = np.einsum(
            "ib...,ja...,ab... -> ij...", self.gammaup3, self.gammaup3, fdown3
        )
        fmixed3 = np.einsum("ij...,jk...->ik...", self.gammaup3, fdown3)
        f00 = np.einsum(
            "i..., j..., ij... -> ...", self.betadown3, self.betadown3, fup3
        )
        f0k = np.einsum("i..., ik... -> k...", self.betadown3, fmixed3)
        fdown4 = np.array(
            [
                [f00, f0k[0], f0k[1], f0k[2]],
    ...
    def weyl_tensor_down4(self, Endown3, Bndown3):
        ...
        ldown4 = self.gdown4 + 2.0 * np.einsum(
            "a..., b... -> ab...", self.ndown4, self.ndown4
        )
        LCudd4 = np.einsum(
            "ec..., d..., dcab... -> eab...",
            self.gup4,
        ...
        Cdown4 = np.einsum("ac..., db... -> abcd...", ldown4, Endown4) - np.einsum(
            "ad..., cb... -> abcd...", ldown4, Endown4
        )
        Cdown4 -= np.einsum("bc..., da... -> abcd...", ldown4, Endown4) - np.einsum(
            "bd..., ca... -> abcd...", ldown4, Endown4
        )
        Cdown4 -= np.einsum(
            "cde..., eab... -> abcd...",
            (
                np.einsum("c..., de... -> cde...", self.ndown4, Bndown4)
                - np.einsum("d..., ce... -> cde...", self.ndown4, Bndown4)
            ),
            LCudd4,
        )
        Cdown4 -= np.einsum(
            "abe..., ecd... -> abcd...",
            (
                np.einsum("a..., be... -> abe...", self.ndown4, Bndown4)
                - np.einsum("b..., ae... -> abe...", self.ndown4, Bndown4)
            ),
            LCudd4,
        )
        return Cdown4
    ...
    def weyl_psi_scalars(self, Cdown4):
        ...
        psi0 = np.einsum(
            "abcd..., a..., b..., c..., d... -> ...", Cdown4, kup, mup, kup, mup
        )
        psi1 = np.einsum(
            "abcd..., a..., b..., c..., d... -> ...", Cdown4, kup, lup, kup, mup
        )
        psi2 = np.einsum(
            "abcd..., a..., b..., c..., d... -> ...", Cdown4, kup, mup, mbup, lup
        )
        psi3 = np.einsum(
            "abcd..., a..., b..., c..., d... -> ...", Cdown4, kup, lup, mbup, lup
        )
        psi4 = np.einsum(
            "abcd..., a..., b..., c..., d... -> ...", Cdown4, mbup, lup, mbup, lup
        )
    ...
    def tetrad_base(self):
        ...
        u2 = v2 - self.vector_projection4(u0, v2) - self.vector_projection4(u1, v2)
        u3 = (
            v3
            - self.vector_projection4(u0, v3)
    ...
    def vector_projection4(self, a, b):
        ...
        return (
            np.einsum("a..., b..., ab... -> ...", a, b, self.gdown4)
            * a
            / np.einsum("a..., b..., ab... -> ...", a, a, self.gdown4)
        )
    ...
    def norm_rank2tensor4(self, fdown4):
        ...
        fup4 = np.einsum("ib..., ja..., ab... -> ij...", self.gup4, self.gup4, fdown4)
    ...
    def levicivita_tensor_down4(self):
        ...
        return self.levicivita_symbol_down4() * np.sqrt(abs(determinant4(self.gdown4)))
...
def inverse3(f):
    ...
    fup = np.array(
        [
            [yy * zz - yz * yz, -(xy * zz - yz * xz), xy * yz - yy * xz],
