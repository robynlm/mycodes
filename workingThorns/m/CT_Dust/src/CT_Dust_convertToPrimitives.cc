...
static void CT_Dust_convertToPrimitives_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL u1 CCTK_ATTRIBUTE_UNUSED = SS1L*pow(DDL,-1)*pow(h,-1);
    
    CCTK_REAL u2 CCTK_ATTRIBUTE_UNUSED = SS2L*pow(DDL,-1)*pow(h,-1);
    
    CCTK_REAL u3 CCTK_ATTRIBUTE_UNUSED = SS3L*pow(DDL,-1)*pow(h,-1);
    
    w_lorentzL = pow(1 + 2*(gu23*u2*u3 + u1*(gu12*u2 + gu13*u3)) + 
      gu11*pow(u1,2) + gu22*pow(u2,2) + gu33*pow(u3,2),0.5);
    
    CCTK_REAL velxL CCTK_ATTRIBUTE_UNUSED = (gu11*u1 + gu12*u2 + 
      gu13*u3)*pow(w_lorentzL,-1);
    
    CCTK_REAL velyL CCTK_ATTRIBUTE_UNUSED = (gu12*u1 + gu22*u2 + 
      gu23*u3)*pow(w_lorentzL,-1);
    
    CCTK_REAL velzL CCTK_ATTRIBUTE_UNUSED = (gu13*u1 + gu23*u2 + 
      gu33*u3)*pow(w_lorentzL,-1);
    
    rhoL = DDL*pow(w_lorentzL,-1)*pow(detg,-0.5);
    
    CCTK_REAL pressL CCTK_ATTRIBUTE_UNUSED = (1 + epsL)*rhoL*eosw;
    /* Copy local copies back to grid functions */
    eps[index] = epsL;
    press[index] = pressL;
    rho[index] = rhoL;
...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CT_Dust_convertToPrimitives
  DECLARE_CCTK_ARGUMENTS_CHECKED(CT_Dust_convertToPrimitives);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    ...
    "HydroBase::vel",
    "HydroBase::w_lorentz"};
  AssertGroupStorage(cctkGH, "CT_Dust_convertToPrimitives", 9, groups);
