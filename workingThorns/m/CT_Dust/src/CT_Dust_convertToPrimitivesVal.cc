...
static void CT_Dust_convertToPrimitivesVal_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(CT_Dust_convertToPrimitivesVal,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL EEloc CCTK_ATTRIBUTE_UNUSED = EEL*pow(detg,-0.5);
    
    CCTK_REAL SSloc1 CCTK_ATTRIBUTE_UNUSED = SS1L*pow(detg,-0.5);
    
    CCTK_REAL SSloc2 CCTK_ATTRIBUTE_UNUSED = SS2L*pow(detg,-0.5);
    
    CCTK_REAL SSloc3 CCTK_ATTRIBUTE_UNUSED = SS3L*pow(detg,-0.5);
    
    CCTK_REAL S2 CCTK_ATTRIBUTE_UNUSED = 2*(gu23*SSloc2*SSloc3 + 
      SSloc1*(gu12*SSloc2 + gu13*SSloc3)) + gu11*pow(SSloc1,2) + 
      gu22*pow(SSloc2,2) + gu33*pow(SSloc3,2);
    
    pressL = 0.5*(EEloc*(-1 + eosw) + pow(-4*eosw*S2 + pow(EEloc,2)*pow(1 
      + eosw,2),0.5));
    
    CCTK_REAL mu CCTK_ATTRIBUTE_UNUSED = pressL*pow(eosw,-1);
    
    epsL = 0.;
    
    CCTK_REAL rhoL CCTK_ATTRIBUTE_UNUSED = mu*pow(1 + epsL,-1);
    
    w_lorentzL = pow((pressL + EEloc)*pow(pressL + mu,-1),0.5);
    
    CCTK_REAL u1 CCTK_ATTRIBUTE_UNUSED = 
      SSloc1*pow(w_lorentzL,-1)*pow(pressL + mu,-1);
    
    CCTK_REAL u2 CCTK_ATTRIBUTE_UNUSED = 
      SSloc2*pow(w_lorentzL,-1)*pow(pressL + mu,-1);
    
    CCTK_REAL u3 CCTK_ATTRIBUTE_UNUSED = 
      SSloc3*pow(w_lorentzL,-1)*pow(pressL + mu,-1);
  ...
  CCTK_ENDLOOP3(CT_Dust_convertToPrimitivesVal);
}
extern "C" void CT_Dust_convertToPrimitivesVal(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CT_Dust_convertToPrimitivesVal
  DECLARE_CCTK_ARGUMENTS_CHECKED(CT_Dust_convertToPrimitivesVal);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CT_Dust_convertToPrimitivesVal_Body");
  }
  if (cctk_iteration % CT_Dust_convertToPrimitivesVal_calc_every != CT_Dust_convertToPrimitivesVal_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "HydroBase::vel",
    "HydroBase::w_lorentz"};
  AssertGroupStorage(cctkGH, "CT_Dust_convertToPrimitivesVal", 8, groups);
  ...
  LoopOverEverything(cctkGH, CT_Dust_convertToPrimitivesVal_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CT_Dust_convertToPrimitivesVal_Body");
  }
}
