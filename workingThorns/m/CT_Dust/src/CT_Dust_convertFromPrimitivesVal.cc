...
static void CT_Dust_convertFromPrimitivesVal_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(CT_Dust_convertFromPrimitivesVal,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    w_lorentzL = pow(1 + 2*(gyzL*velyL*velzL + velxL*(gxyL*velyL + 
      gxzL*velzL)) + gxxL*pow(velxL,2) + gyyL*pow(velyL,2) + 
      gzzL*pow(velzL,2),-0.5);
    
    CCTK_REAL h CCTK_ATTRIBUTE_UNUSED = (1 + epsL)*(1 + eosw);
    
    CCTK_REAL DDL CCTK_ATTRIBUTE_UNUSED = rhoL*w_lorentzL;
    
    CCTK_REAL EEL CCTK_ATTRIBUTE_UNUSED = -pressL + 
      rhoL*h*pow(w_lorentzL,2);
    
    CCTK_REAL SS1L CCTK_ATTRIBUTE_UNUSED = rhoL*(gxxL*velxL + gxyL*velyL + 
      gxzL*velzL)*w_lorentzL*h;
    
    CCTK_REAL SS2L CCTK_ATTRIBUTE_UNUSED = rhoL*(gxyL*velxL + gyyL*velyL + 
      gyzL*velzL)*w_lorentzL*h;
    
    CCTK_REAL SS3L CCTK_ATTRIBUTE_UNUSED = rhoL*(gxzL*velxL + gyzL*velyL + 
      gzzL*velzL)*w_lorentzL*h;
    /* Copy local copies back to grid functions */
    DD[index] = DDL;
    EE[index] = EEL;
    SS1[index] = SS1L;
    SS2[index] = SS2L;
    SS3[index] = SS3L;
    w_lorentz[index] = w_lorentzL;
  }
  CCTK_ENDLOOP3(CT_Dust_convertFromPrimitivesVal);
}
extern "C" void CT_Dust_convertFromPrimitivesVal(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CT_Dust_convertFromPrimitivesVal
  DECLARE_CCTK_ARGUMENTS_CHECKED(CT_Dust_convertFromPrimitivesVal);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CT_Dust_convertFromPrimitivesVal_Body");
  }
  if (cctk_iteration % CT_Dust_convertFromPrimitivesVal_calc_every != CT_Dust_convertFromPrimitivesVal_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "HydroBase::vel",
    "HydroBase::w_lorentz"};
  AssertGroupStorage(cctkGH, "CT_Dust_convertFromPrimitivesVal", 9, groups);
  ...
  LoopOverEverything(cctkGH, CT_Dust_convertFromPrimitivesVal_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CT_Dust_convertFromPrimitivesVal_Body");
  }
}
