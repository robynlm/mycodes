...
static void CT_Dust_addToTmunu_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL eTttL CCTK_ATTRIBUTE_UNUSED = (pressL - 
      0.0397887357729738339422209408431*Lambda)*(bsq - pow(alpL,2)) + (pressL 
      + mu)*pow(u0l,2);
    
    CCTK_REAL eTtxL CCTK_ATTRIBUTE_UNUSED = (betaxL*gxxL + betayL*gxyL + 
      betazL*gxzL)*(pressL - 0.0397887357729738339422209408431*Lambda) + 
      (pressL + mu)*u0l*u1;
    
    CCTK_REAL eTtyL CCTK_ATTRIBUTE_UNUSED = (betaxL*gxyL + betayL*gyyL + 
      betazL*gyzL)*(pressL - 0.0397887357729738339422209408431*Lambda) + 
      (pressL + mu)*u0l*u2;
    
    CCTK_REAL eTtzL CCTK_ATTRIBUTE_UNUSED = (betaxL*gxzL + betayL*gyzL + 
      betazL*gzzL)*(pressL - 0.0397887357729738339422209408431*Lambda) + 
      (pressL + mu)*u0l*u3;
    
    CCTK_REAL eTxxL CCTK_ATTRIBUTE_UNUSED = gxxL*(pressL - 
      0.0397887357729738339422209408431*Lambda) + (pressL + mu)*pow(u1,2);
    
    CCTK_REAL eTxyL CCTK_ATTRIBUTE_UNUSED = gxyL*(pressL - 
      0.0397887357729738339422209408431*Lambda) + (pressL + mu)*u1*u2;
    
    CCTK_REAL eTxzL CCTK_ATTRIBUTE_UNUSED = gxzL*(pressL - 
      0.0397887357729738339422209408431*Lambda) + (pressL + mu)*u1*u3;
    
    CCTK_REAL eTyyL CCTK_ATTRIBUTE_UNUSED = gyyL*(pressL - 
      0.0397887357729738339422209408431*Lambda) + (pressL + mu)*pow(u2,2);
    
    CCTK_REAL eTyzL CCTK_ATTRIBUTE_UNUSED = gyzL*(pressL - 
      0.0397887357729738339422209408431*Lambda) + (pressL + mu)*u2*u3;
    
    CCTK_REAL eTzzL CCTK_ATTRIBUTE_UNUSED = gzzL*(pressL - 
      0.0397887357729738339422209408431*Lambda) + (pressL + mu)*pow(u3,2);
    /* Copy local copies back to grid functions */
    eTtt[index] = eTttL;
    eTtx[index] = eTtxL;
    eTty[index] = eTtyL;
...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CT_Dust_addToTmunu
  DECLARE_CCTK_ARGUMENTS_CHECKED(CT_Dust_addToTmunu);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    ...
    "HydroBase::vel",
    "HydroBase::w_lorentz"};
  AssertGroupStorage(cctkGH, "CT_Dust_addToTmunu", 8, groups);
